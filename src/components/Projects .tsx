...
function Thumbnail({ project }: { project: Project }) {
  ...
  return (
    <div className="relative h-36 w-full shrink-0 bg-sand">
      {showImage ? (
...
function ProjectCard({ project, onOpen }: { project: Project; onOpen: () => void }) {
  return (
    <article className="flex h-[330px] w-[260px] shrink-0 snap-start flex-col overflow-hidden rounded-2xl border-2 border-ink bg-paper shadow-[5px_5px_0_var(--color-ink)]">
...
export default function Projects() {
  ...
  const scrollBy = (direction: 1 | -1) => trackRef.current?.scrollBy({ left: direction * 280, behavior: "smooth" });
